...
import com.spoonner.alex.datastruct.views.DSViewListArray;
...
public class DSPanelListArray extends DSPanel {
    ...
    public DSPanelListArray(DSWindow window) {
        super(window);
        int i;
...
//            insertBeforeButton[i].setForeground(iteratorColors[i]);
        ...
        lengthButton.addActionListener(event -> {
        ...
        getIteratorButton.addActionListener(event -> {
            if (activeIterators < (MAXITERATOR)) {
                ...
                activeIterators++;
                if (activeIterators == MAXITERATOR)
                    DSPanelListArray.this.getIteratorButton.setEnabled(false);
...
//        this.add(iter1, BorderLayout.NORTH);
        this.add(view = listView = new DSViewListArray(), BorderLayout.CENTER);
    ...
    private void AddActionListenButtonField(final JButton button, final int iteratornum, final int action,
        ...
        button.addActionListener(event -> {
            if (field.getText().length() != 0) {
                    animate(action, iteratornum, extractString(field.getText(),6));
                field.setText("");
                changeDone();
            }
        });
    ...
    private void AddActionListenButtonField2(final JButton button, final int iteratornum, final int action,
        ...
        button.addActionListener(event -> {
            if (field.getText().length() != 0) {
                ...
                if (insertelem < 1000) {
                    animate(action, iteratornum, insertelem);
                }
                field.setText("");
                changeDone();
            }
