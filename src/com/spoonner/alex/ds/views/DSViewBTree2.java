package com.spoonner.alex.ds.views;


import com.spoonner.alex.ds.shapes.DSShapeRectMultiLabel;
import com.spoonner.alex.xj.appkit.gview.base.Vector2D;
import com.spoonner.alex.xj.appkit.gview.object.GElement;
import com.spoonner.alex.xj.appkit.gview.object.GElementLabel;
...
public class DSViewBTree2 extends DSView {
    ...
    protected void insert(String insertElem, BTreeNode tree) {
        int i;
        ...
        if (tree.leaf) {
                ...
                insert(insertElem, tree.children[i]);
            }
    }
...
/*        int childindex = findChildIndex(tree, tree.parent);
        if ((childindex > 0) && parent.children[childindex-1].numkeys < MAX_KEYS)
            StealRight(parent,childindex-1);
        else if (childindex < parent.numkeys && parent.children[childindex+1].numkeys < MAX_KEYS) {
            StealLeft(parent,childindex+1);
        } else { */
            BTreeNode newtree = split(tree);
            InsertRepair(newtree);
       /* } */
    }
    ...
    private void delete(String removeitem, BTreeNode newroot) {
        int i,j;
        ...
        if (newroot.leaf) {
            ...
            if (newroot.numkeys < MIN_KEYS)
                DeleteRepair(newroot);
            return;
        }
        ...
        if (i == newroot.numkeys) {
            ...
            delete(removeitem,newroot.children[i]);
    ...
    private void DeleteRepair(BTreeNode tree) {
        tree.display.setColor(Color.GREEN);
        repaintwait();
        tree.display.setColor(Color.BLACK);
        ...
        int childindex = findChildIndex(tree, tree.parent);
        if ((childindex > 0) && parent.children[childindex-1].numkeys > MIN_KEYS)
            StealLeft(parent,childindex);
        else if (childindex < parent.numkeys && parent.children[childindex+1].numkeys > MIN_KEYS) {
            StealRight(parent,childindex);
        } else if (childindex > 0) {
            BTreeNode combined = CombineLeft(parent,childindex);
            DeleteRepair(combined.parent);
        } else {
            BTreeNode combined = CombineRight(parent,childindex);
            DeleteRepair(combined.parent);
    ...
    private BTreeNode findLargest(BTreeNode tree) {
        ...
        return findLargest(tree.children[tree.numkeys]);
    }
    ...
    private BTreeNode findSmallest(BTreeNode tree) {
        ...
        return findSmallest(tree.children[0]);
    }
    ...
    protected class BTreeNode {
        ...
        public BTreeNode() {
            int i;
            numkeys = 0;
            children = new BTreeNode[MAX_CHILDREN+1];
            for (i = 0; i < MAX_CHILDREN+1; i++)
                children[i] = null;
            keys = new String[MAX_KEYS+1];
